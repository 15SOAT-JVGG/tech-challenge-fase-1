package br.com.fiap.postech.soat16.fase1.part.adapter.in.rest.openapi;
...
public interface PartControllerDocs {
    ...
    @APIResponse(
            responseCode = "200",
            description = "Parts/supplies returned successfully",
            content = @Content(
    ...
    @Path("/{id}")
    @Operation(
            summary = "Find part/supply by ID",
            description = "Returns a single part/supply by its identifier.")
    @APIResponse(
            responseCode = "200",
            description = "Part/supply found",
            content = @Content(
                    mediaType = MediaType.APPLICATION_JSON,
    ...
    Uni<PartResponseDto> findById(
            @Parameter(
                    name = "id",
                    description = "Part/supply identifier",
                    required = true,
                    in = ParameterIn.PATH)
    ...
    @POST
    @Operation(
            summary = "Register new part/supply",
            description = "Creates a new part/supply record. Restricted to ADMIN.")
    @APIResponse(
            responseCode = "201",
            description = "Part/supply created successfully",
            content = @Content(
                    mediaType = MediaType.APPLICATION_JSON,
                    schema = @Schema(implementation = PartResponseDto.class)))
    ...
    @Path("/{id}")
    @Operation(
            summary = "Update part/supply",
            description = "Fully replaces the data of a part/supply. Restricted to ADMIN.")
    @APIResponse(
            responseCode = "200",
            description = "Part/supply updated successfully",
            content = @Content(
                    mediaType = MediaType.APPLICATION_JSON,
                    schema = @Schema(implementation = PartResponseDto.class)))
    @APIResponse(responseCode = "404", description = "Part/supply not found")
    ...
    @Operation(
            ...
            description = "Operation restricted to ADMIN. Mechanics consume stock indirectly when adding parts "
                    + "to a service order; manual inbound/outbound adjustments (purchase, return, loss) are an "
    ...
    Uni<PartResponseDto> adjustStock(
            ...
            @Parameter(
                    name = "adjustment",
                    description = "Quantity to adjust (positive=inbound, negative=outbound)",
                    required = true,
    ...
    @Path("/{id}")
    @Operation(
            summary = "Delete part/supply",
            description = "Permanently removes a part/supply by its identifier. Restricted to ADMIN.")
